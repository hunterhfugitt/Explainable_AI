...
def _cluster_this(scaling1,scaling2,scaling3,scaling4,list1,list2,list3,cached_value):
    ...
    if(os.path.exists(f'{place}\\static\\images\\{cached_value}.svg')):
        ...
        with open(f"static\\test_user_input\\{cached_value}_list.pickle", "wb") as fp:
        ...
        for c in list_values:
            for d in list_values:
                if a != 0 or b!= 0 or c !=0 or d!= 0:
                    _cluster_this(a,b,c,d,list1,list2,list3,f'co{a},ty{b},te{c},fl{d}')
...
place = os.getcwd()
for x in list_values:
    file = open(f'{place}\\static\\test_user_input\\color{x}_list.pickle','rb')
    list_to_add = pickle.load(file)
    list_before = list_to_add[0]
    list_after = list_to_add[2]
    sum = 0.0
    for count,each in enumerate(list_to_add[0]):
        # print(len(each))
        # #print(each[0])
        # print(len(list_after[count]))
        # #print(changed)
        # print(each[0])
        # print(list_after[count][0])
        # print(len(each[0]))
        # print(len(list_after[count][0]))
        difference  = spatial.distance.cosine(each[0],list_after[count][0])
        sum = sum + difference
    print(f'color{str(x)} is '+ str(sum))
