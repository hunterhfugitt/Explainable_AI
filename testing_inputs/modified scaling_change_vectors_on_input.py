...
def _cluster_this(scaling1,scaling2,scaling3,scaling4,list1,list2,list3,cached_value):
    ...
    scalings = []
    scalings.append(scaling1)
    scalings.append(scaling2)
    scalings.append(scaling3)
    scalings.append(scaling4)
    weights = []
    if(os.path.exists(f'{place}\\static\\images\\{cached_value}.svg')):
        ...
        with open(f"static\\test_user_input\\modified_version3_{cached_value}_list.pickle", "wb") as fp:
